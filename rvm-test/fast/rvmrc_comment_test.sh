...
rvm alias list            # match=/default => ruby-3.1.3/
...
cd
rvm use 3.1.0
echo "rvm use 3.1.3" > "$d/.rvmrc"
rvm rvmrc trust "$d"      # match=/ as trusted$/
cd "$d"                   # env[GEM_HOME]=/3.1.3$/

## load ruby@gemset
cd
rvm use 3.1.0
echo "rvm use 3.1.3@vee --create" > "$d/.rvmrc"
rvm rvmrc trust "$d"      # match=/ as trusted$/
cd "$d"                   # env[GEM_HOME]=/3.1.3@vee$/
