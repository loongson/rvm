...
: generate
mcd $d/a
rvm use 3.1.0@versions-conf-a --create --versions-conf --install
rvm current             # match=/^ruby-3.1.0@versions-conf-a$/
[[ -f .versions.conf ]] # status=0

mcd $d/b
rvm use 3.0.5@versions-conf-b --create --versions-conf --install
rvm current             # match=/^ruby-3.0.5@versions-conf-b$/
[[ -f .versions.conf ]] # status=0
...
: clean
rvm 3.1.0 do rvm --force gemset delete versions-conf-a
rvm 3.0.5 do rvm --force gemset delete versions-conf-b
rm -rf $d
