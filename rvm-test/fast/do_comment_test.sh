...
rvm use 3.1.0 --install # status=0

: do
rvm 8.9.9 do rvm gemdir # status=1; match=/is not installed/
rvm 3.0.5 do rvm gemdir # status=0; match=/3.0.5/
rvm 3.0.5@test0 do rvm gemdir # status=2; match=/Gemset .* does not exist/
rvm 3.0.5@test1 do rvm gemdir # status=0; match=/3.0.5@test1/
rvm 3.0.5@test2 do rvm gemdir # status=0; match=/3.0.5@test2/
...
ver=3.1.0
mkdir -p $d/3.1.0
echo "3.1.0" > $d/3.1.0/.ruby-version

: absolute directory
rvm in $d/3.1.0 do rvm info ruby  # status=0; match=/version: *"3.1.0/
rvm in $d/3.1.0 do ruby --version # status=0; match=/^ruby 3.1.0/

: relative directory
cd $d
rvm in 3.1.0 do rvm info ruby  # status=0; match=/version: *"3.1.0/
rvm in 3.1.0 do ruby --version # status=0; match=/^ruby 3.1.0/

: current directory
cd $d/3.1.0
rvm . do rvm info ruby  # status=0; match=/version: *"3.1.0/
rvm . do ruby --version # status=0; match=/^ruby 3.1.0/
