...
rvm alias create default 3.0.5
rvm alias list            # match=/default => ruby-3.0.5/
...
cd
rvm use 3.0.1
echo "rvm use 3.0.5" > "$d/.rvmrc"
rvm rvmrc trust "$d"      # match=/ as trusted$/
cd "$d"                   # env[GEM_HOME]=/3.0.5$/

## load ruby@gemset
cd
rvm use 3.0.1
echo "rvm use 3.0.5@vee --create" > "$d/.rvmrc"
rvm rvmrc trust "$d"      # match=/ as trusted$/
