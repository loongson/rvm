...
ver=3.0.1
mkdir -p $d/3.0.1
echo "3.0.1" > $d/3.0.1/.ruby-version

: absolute directory
rvm in $d/3.0.1 do rvm info ruby  # status=0; match=/version: *"3.0.1/
rvm in $d/3.0.1 do ruby --version # status=0; match=/^ruby 3.0.1/

: relative directory
cd $d
rvm in 3.0.1 do rvm info ruby  # status=0; match=/version: *"3.0.1/
rvm in 3.0.1 do ruby --version # status=0; match=/^ruby 3.0.1/

: current directory
cd $d/3.0.1
rvm . do rvm info ruby  # status=0; match=/version: *"3.0.1/
rvm . do ruby --version # status=0; match=/^ruby 3.0.1/
