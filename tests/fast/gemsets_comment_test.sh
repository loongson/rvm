...
rvm use 3.0.1                            # status=0
rvm current                              # match=/^ruby-3.0.1$/
rvm gemset list                          # match!=/test_gemset/

: rvm ... do
rvm 3.0.1 do rvm gemset create test_gemset            # status=0 ; match=/gemset created/
rvm 3.0.1 do rvm gemset list                          # match=/test_gemset/; match!=/other_gems/
rvm 3.0.1 do rvm --force gemset delete test_gemset    # status=0
...
rvm --force gemset delete test_gemset    # status=0
rvm 3.0.1 do rvm gemset create test_gemset            # status=0 ; match=/gemset created/; match=/test-rvm_gemsets_path/
rvm 3.0.1 do rvm gemset list                          # match=/test_gemset/; match!=/other_gems/; match=/test-rvm_gemsets_path/
rvm 3.0.1 do rvm --force gemset delete test_gemset    # status=0
...
rvm --force --debug gemset empty test_gemset # status=0 ; match=/GEM_HOME=.*ruby-3.0.1@test_gemset/
rvm gemset use test_gemset               # status=0 ; match=/Using /
gem list                                 # match!=/haml/
